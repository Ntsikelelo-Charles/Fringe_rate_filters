...
E_W_baseline_cut_array=np.array(["31_m","100_m"])
# E_W_baseline_cut_array=np.array(["30_m","50_m","35_m"])
# fixed_width_array=np.array(["","_0_2mHz","_0_1mHz","_0_05mHz"])
fixed_width_array=np.array([""])

# noise_mode_array=np.array(["with_noise","no_noise"])

noise_mode_array=np.array(["with_noise"])
for bls_c in range (len(E_W_baseline_cut_array)):
    
    E_W_baseline_cut=E_W_baseline_cut_array[bls_c]
    print("baseline cut EW is "+E_W_baseline_cut)
    
    bls_EW_cut=0
    if E_W_baseline_cut=="30_m":
        bls_EW_cut=30
    if E_W_baseline_cut=="31_m":
        bls_EW_cut=31 

    if E_W_baseline_cut=="50_m":
        bls_EW_cut=50 
        
    if E_W_baseline_cut=="100_m":
        bls_EW_cut=100     
    
    for  wd in range (len(fixed_width_array)):
        fixed_width=fixed_width_array[wd]
        ...
        for i in range(len(mode_array)):
            ...
            hd = hc.io.HERAData("/net/sinatra/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Raw_data_filtered_Gaussian_non_redundant_with_noise_"+mode+"_2h"+fixed_width+".uvh5")
            ...
            hd2 = hc.io.HERAData("/net/sinatra/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Model_complete_filtered_Gaussian_non_redundant_"+mode+"_2h"+fixed_width+".uvh5")
            ...
            for k in data:
                ...
                if bl_len_EW <bls_EW_cut: 
                    cut+=1

                    noise_wgts[k] = np.ones(noise_wgts[k].shape)*1e-40
            print("number base line cut=="+str(cut))
            # perform logcal
            ...
            np.save("/home/ntsikelelo/non_redundancy_sim/lincal_gains_first_order_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",gains_all)
            ...
            np.save("/home/ntsikelelo/non_redundancy_sim/redcal_chisq_first_order_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",rc_red_chisq)
            ...
            print("abscal number base line cut=="+str(cut))
            # get noise wgts (use whatever noise RMS you put into the data)
            ...
            np.save("/home/ntsikelelo/non_redundancy_sim/gains_first_order_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",gains_all)
            ...
            np.save("/home/ntsikelelo/non_redundancy_sim/abscal_chisq_zeroth_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",red_chisq)
            ...
            np.save("/home/ntsikelelo/non_redundancy_sim/mdl_wedge_fft_first_order_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",mdl_wedge_fft)
            np.save("/home/ntsikelelo/non_redundancy_sim/cal_wedge_fft_first_order_vis_"+filter_type+"_"+mode+"_2h_"+fixed_width+"_"+E_W_baseline_cut+".npy",cal_wedge_fft) 